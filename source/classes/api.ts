...
export class Api {
    ...
    hooks: {
      beforeRequest: [
        (options: NormalOptions) => {
          ...
          options.unityAgent
            ? (options.headers['User-Agent'] = `UnityPlayer/${this.unityVersion} (UnityWebRequest/1.0, libcurl/7.52.0-DEV)`)
            : null
  ...
  private async getLauncherVersion() {
    this.launcher.post('launcher/GetLauncherDistrib').then(
      (result: any) => {
